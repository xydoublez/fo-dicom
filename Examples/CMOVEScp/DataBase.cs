...
namespace ZYCMOVEScp
{
    ...
    public class test
    {
        ...
        {
            return false;
            SqlInfoMessageEventArgs ar = null;
        }
    }
    public class DataBase
    {
        static string constr = Encrypt.Decrypt(System.Configuration.ConfigurationManager.AppSettings["ZYPACSDB"], "ihepass");
        private static string dbType = System.Configuration.ConfigurationManager.AppSettings["DbType"].ToString();
        public static  DataSet GetImage(string studySopinstanceUid)
        {
            ...
            if (dbType == "SqlServer")
            {
                 sql = "SELECT REFERENCE_FILE FROM ZYPACSDB.DBO.VIEW_PATIENT_IMAGES WHERE STUDY_INSTANCE_UID=@studySopinstanceUid";
            ...
            if (dbType == "Oracle")
            {
                 sql = "SELECT REFERENCE_FILE FROM VIEW_PATIENT_IMAGES WHERE STUDY_INSTANCE_UID=:studySopinstanceUid";
